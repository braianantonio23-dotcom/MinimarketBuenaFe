...
const Dashboard: React.FC<DashboardProps> = ({ products, invoices }) => {
  ...
  const generateChartData = (): FinancialChartData[] => {
    ...
    let chartData: FinancialChartData[] = Array.from(dataMap.entries())
        .map(([date, d]) => ({ 
            date, 
            sales: d.sales, 
    ...
    if (chartData.length > 2) {
        ...
        const avgSales = lastPoints.reduce((sum, p) => sum + (p.sales || 0), 0) / lastPoints.length;
  ...
  const inventoryChartData = products.map(p => ({
    name: p.name, // Use full name to avoid duplicates
